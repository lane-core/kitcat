...
open import Prim.Type
  using (Level; Type; SSet; SSetω)
  renaming (funcomp to infixr 40 _∘_)
...
{-# DISPLAY transp (∂.contract A i) i = coei1 A i #-}
{-# DISPLAY transp (∂.sym A) i0 = coe10 A #-}
{-# DISPLAY transp (∂.sym (∂.contract A i)) i = coe1i A i #-}
...
fill A φ i u = comp (∂.extend A i) (φ ∨ ~ i) sys
  module fill where
    sys : Partials (φ ∨ ~ i) (λ j → A (i ∧ j))
    sys j (i = i0) = u i0 1=1
    sys j (j = i0) = u i0 1=1
    sys j (φ = i1) = u (i ∧ j) 1=1
...
transport-filler A x i = kext i (λ k _ → A (i ∧ k)) (λ _ a → a) (λ k → coe0i A k x)
