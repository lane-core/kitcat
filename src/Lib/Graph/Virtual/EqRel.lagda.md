...
record is-eq-rel {u} {A : Type u} v (R : A → A → Type v) : Type (u ⊔ v) where
  field
...
is-right-divisible : {a x y : Ob} (p : x ~> y) (q : a ~> y) → Type (v ⊔ w)
is-right-divisible p q = is-contr (fibroid p q)
...
record _≈_ {x z} (f g : x ~> z) : Type (u ⊔ v ⊔ w) where
  field
    fwd : f => g
    back : f => g
    pre : {h : x ~> z} (H : f => h) → is-contr (Σ γ ∶ g => h , fwd ⊚ γ ≡ H)
    post : {h : x ~> z} (H : h => g) → is-contr (Σ γ ∶ h => f , γ ⊚ fwd ≡ H)
