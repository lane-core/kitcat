...
open import Lib.Core.Kan using (hcomp; hfill; transport-filler; _∙_)
...
module V-graph {u v w} ((G , V) : V-graph u v w) where
  ...
  eql-path : ∀ {x y z} {f : x ~> y} {g : y ~> z} {r s : x ~> z}
            → f ⨾ g => r → f ⨾ g => s → r => s
  eql-path f g = lift-path f (sym (to-path f) ∙ to-path g)
  ...
  weak-hsym : ∀ {x y z} {f : x ~> y} {g : y ~> z} {s : x ~> z}
            → f ⨾ g => s → s => f ⨾ g
  weak-hsym {s} α = transport (λ i → s => to-path α (~ i)) (loop α)

  weak-vconcat : ∀ {w x y z} {f : w ~> x} {f' : x ~> z} {g : w ~> y} {g' : y ~> z} {s : w ~> z}
               → f ⨾ f' => g ⨾ g' → g ⨾ g' => s → f ⨾ f' => s
  weak-vconcat {f} {f'} H K = transport (λ i → f ⨾ f' => to-path K i) H
  ...
  module Overlap {w x y z} {f : w ~> x} {g : x ~> y} {h : y ~> z} where
    ...
    conj : {s : w ~> y} {r : x ~> z}
         ...
         → (f ⨾ g) ⨾ h => f ⨾ (g ⨾ h)
         → s ⨾ h => f ⨾ r
    conj H K = transport (λ i → (to-path H i ⨾ h) => (f ⨾ to-path K i))

    lcross : {s : w ~> y} → f ⨾ g => s
           → (f ⨾ g) ⨾ h => f ⨾ (g ⨾ h)
           → s ⨾ h => f ⨾ (g ⨾ h)
    lcross H = transport (λ i → (to-path H i ⨾ h) => (f ⨾ g ⨾ h))

    -- Keep left concrete, abstract right
    rcross : {r : x ~> z} → g ⨾ h => r
           → (f ⨾ g) ⨾ h => f ⨾ (g ⨾ h)
           → (f ⨾ g) ⨾ h => f ⨾ r
    rcross K = transport (λ i → ((f ⨾ g) ⨾ h) => (f ⨾ to-path K i))
