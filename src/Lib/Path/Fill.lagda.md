...
module cat {A : Type u} where
  module _ {x y z : A} (p : x ≡ y) (q : y ≡ z) where
    sys : (i : I) → Partials (∂ i) (λ _ → A)
    sys i k (i = i0) = x
    sys i k (k = i0) = p i
    sys i k (i = i1) = q k
  ...
  cfill : {x y z : A} (p : x ≡ y) (q : y ≡ z)
        → hsym p ≡ q :: ∂.square _≡_ rfl (p ∙ q)
  ...
  lpush : {w x y z : A} (p : x ≡ w) (q : x ≡ y) (r : y ≡ z) (s : w ≡ z)
        → Square p q r s → Square rfl q r (p ∙ s)
  ...
  rpush : {w x y z : A} (p : x ≡ w) (q : x ≡ y) (r : y ≡ z) (s : w ≡ z)
        → Square p q r s → Square p rfl (q ∙ r) s
  ...
  rpop : {w x y z : A} (p : x ≡ w) (q : x ≡ y) (r : y ≡ z) (s : w ≡ z)
       → Square p rfl (q ∙ r) s → Square p q r s
  ...
  compositor : {w x y z : A}
             → (p : w ≡ x) (q : x ≡ y) (r : y ≡ z)
             → Square rfl p (cat q r) (cat2 (hsym p) q r)
  ...
  unique : {x y z : A}
         ...
         → r ≡ p ∙ q
  unique {p} {q} r sq i =
    Path-filler.unique rfl p q (r , sq) (p ∙ q , fill p q) i .fst

  Tri : {x y z : A} → x ≡ y → y ≡ z → x ≡ z → Type u
  Tri p q s = s ≡ p ∙ q

  unitl : {x y : A} (p : x ≡ y) → rfl ∙ p ≡ p
  unitl p i j = rfill rfl p j (~ i)

  unitr : {x y : A} (p : x ≡ y) → p ∙ rfl ≡ p
  unitr p i j = fill p rfl j (~ i)

  invl : {x y : A} (p : x ≡ y) → cat (hsym p) p ≡ rfl
  ...
  invr : {x y : A} (p : x ≡ y) → cat p (hsym p) ≡ rfl
  invr p = invl (hsym p)

  assoc : {w x y z : A} → (p : w ≡ x) (q : x ≡ y) (r : y ≡ z)
        → cat p (cat q r) ≡ cat (cat p q) r
  assoc {y} p q r k = (λ i → fill p q i k) ∙ (λ i → rfill q r i (~ k))

  hsqueeze : {x y : A} {p q : x ≡ y} → p ∙ rfl ≡ rfl ∙ q → p ≡ q
  hsqueeze {p} {q} h = cat2 (unitr p) h (unitl q)

  vsqueeze : {x y : A} {p q : x ≡ y} → rfl ∙ p ≡ q ∙ rfl → p ≡ q
...
hcat : {A : Type u} (B : A → Type v) {x y z : A}
     → {a : B x} {b : B y} {c : B z}
     → {p : x ≡ y} {q : y ≡ z}
     → a ≡ b :: ∂.line B p
     → b ≡ c :: ∂.line B q
