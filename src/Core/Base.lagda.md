...
iconst : {@0 A : Type u} → A → I → A
iconst a _ = a
{-# INLINE iconst #-}

iflip : {@0 A : I → I → Type u} → (∀ i j → A i j) → ∀ i j → A j i
iflip f = λ j i → f i j
{-# INLINE iflip #-}
...
iota-notation : {A : I → Type u} → (∀ i → A i) → (∀ i → A i)
iota-notation f = f
...
_[_≡_] pathp-syntax : (A : I → Type u) → A i0 → A i1 → Type u
_[_≡_] = PathP
pathp-syntax = PathP
...
refl : {A : Type u} {x : A} → x ≡ x
refl {x = x} = λ _ → x
{-# INLINE refl #-}

sym : {A : I → Type u} {x : A i0} {y : A i1}
    → PathP A x y → PathP (λ i → A (~ i)) y x
sym p i = p (~ i)
{-# INLINE sym #-}

ap : {@0 A : Type u} {@0 B : A → Type v}
   → (f : ∀ x → B x) {x y : A} (p : x ≡ y)
   → PathP (λ i → B (p i)) (f x) (f y)
ap f p i = f (p i)
{-# INLINE ap #-}

ap-era : {@0 A : Type u} {@0 B : A → Type v} (f : ∀ (@0 x) → B x)
    → {x y : A} (@0 p : x ≡ y)
    → PathP (λ i → B (p i)) (f x) (f y)
ap-era f p i = f (p i)
{-# INLINE ap-era #-}

apd : {@0 A : I → Type u} {@0 B : ∀ i → A i → Type v}
    → (f : ∀ i (a : A i) → B i a)
...
Irr-is-prop : {A : Type u} → is-prop (Irr A)
...
_~ᵢ_ : {A : I → Type u} → (∀ i → A i) → (∀ i → A i) → Type u
f ~ᵢ g = ∀ i → f i ≡ g i

_∘i_ : {v : I → Level} {A : I → Type u} {B : ∀ i → A i → Type (v i)}
     → (g : ∀ i (a : A i) → B i a)
     → (f : ∀ i → A i)
     → ∀ i → B i (f i)
...
module _ {A : Type u} {a00 a01 a10 a11 : A}
...
_~_ : {A : Type u} {B : A → Type v}
    → (f g : (x : A) → B x) → Type (u ⊔ v)
f ~ g = (x : _) → f x ≡ g x
infix 3 _~_

funext : {@0 A : Type u} {@0 B : A → Type v} {f g : (x : A) → B x}
       → ((x : A) → f x ≡ g x) → f ≡ g
funext p i x = p x i
{-# INLINE funext #-}

funexti : {@0 A : I → Type u} {f g : (i : I) → A i}
        → ((i : I) → f i ≡ g i) → f ≡ g
funexti p i j = p j i
{-# INLINE funexti #-}

happly : {@0 A : Type u} {@0 B : A → Type v} {f g : (x : A) → B x}
       → f ≡ g → (x : A) → f x ≡ g x
happly p x i = p i x
{-# INLINE happly #-}
